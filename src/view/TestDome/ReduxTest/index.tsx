...
export const TestContext  = React.createContext<ITextConetxt>({value:"原始值",setValue:undefined,drawerProps:{ className:"custom-default"}})

const CommonDrawer:FC<PropsWithChildren<DrawerProps>> = ({children,className:pclassName,...props})=>{
    
    const {drawerProps={ className:""}} = useContext(TestContext)
    return <Drawer {...drawerProps} {...props} className={`${drawerProps.className} ${pclassName}`}>{children}</Drawer>
}
...
const Test2: React.FC<IAppProps> = () => {
   ...
   return <div style={{
     ...
     <CommonDrawer className="custom-2" visible={open}        title="自定义弹窗" 
  onClose={()=>setOpen(false)} getContainer={false} width={"100%"}>
        <div>
        <p>Some contents...</p>
          <p>Some contents...</p>
          <p>Some contents...</p>
     ...
     <Button onClick={()=>result.setValue?.("context" +Math.random())}>自定义context</Button>
     组件2 ：{result.value}
...
const Test3: React.FC<IAppProps> = () => {
    ...
    return <div style={{
      ...
      <CommonDrawer className="custom-3" visible={open}        title="自定义弹窗" 
   onClose={()=>setOpen(false)} getContainer={false} width={"100%"}>
         <div>
         <p>Some contents...</p>
           <p>Some contents...</p>
