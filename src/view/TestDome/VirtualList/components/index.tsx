...
const App = forwardRef<IVirtualListForward, IProps>(
  (
    {
      data = [],
      layout = "vertical",
    ...
    const [itmeStyle] = useState<CSSProperties>(
      layout === "horizontal" ? { width: itemSize } : { height: itemSize }
    );
    ...
    const nodeAttrs = useMemo(() => {
      return isHorizontal
        ? { scroll: "scrollLeft", client: "clientWidth" }
        : { scroll: "scrollTop", client: "clientHeight" };
    }, [isHorizontal]);
    ...
    useImperativeHandle(ref, () => ({
      ...
      goPage: (page: number) => {
        ...
        if (viewLen > target[nodeAttrs.client]) {
          // 真实要显示的大于可视区域，要减去一个
    ...
    const updateViews = useCallback(
      (target: HTMLDivElement) => {
        requestAnimationFrame(() => {
          ...
          if (
            scrollLen > 0 &&
            (endIndex >= totals || endIndex + viewCounts.current >= totals)
          ) {
            outScoll = viewCounts.current * itemSize - target[nodeAttrs.client];
          }

          const transform = isHorizontal
            ? `translate3d(${scrollLen}px,0px, 0px)`
            : `translate3d(0px, ${scrollLen}px, 0px)`;
          ...
          setviewListStyle(
            isHorizontal
              ? {
                  width: listLen,
                  transform: `translate3d(-${outScoll}px,0px, 0px)`,
                }
              : {
                  height: listLen,
                  transform: `translate3d(0px, -${outScoll}px, 0px)`,
                }
          );
