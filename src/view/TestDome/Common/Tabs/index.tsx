...
const App: React.FC<IATabsProps> = ({
  data = [],
  className = "",
  style = {},
  defaultKey = "1",
  ...
  return (
    <div className={`height-full page-padding-1 ${className}`} style={style}>
      <Tabs
        className="height-full block-bg-1 custom-tabs-warp"
      ...
      >
        {data.map((item) => {
          ...
          return (
            <TabPane key={tabKey} tabKey={tabKey} {...itmeProps}>
              <div  className="height-full block-padding-1 custom-tabs-children">
                {children}
              </div>
            </TabPane>
          );
